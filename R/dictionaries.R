...
# @rdname dictionary @export NEED TO ADD A VALIDATOR
setClass("dictionary", contains = c("list"), slots = c(concatenator = "character", format = "charNULL", file = "charNULL"), prototype = prototype(concatenator = " ", format = NULL, file = NULL))
...
#' @export
setMethod("show", "dictionary", function(object) {
    cat("Dictionary object with", length(object), "key entries.\n")
    keys <- names(object)
    lapply(seq_along(object), function(i, object, keys) cat(" - ", keys[i], ": ", paste(object[[i]], collapse = ", "), "\n", sep = ""), object = object, keys = names(object))
...
#' @export
dictionary <- function(x = NULL, file = NULL, format = NULL, concatenator = " ", toLower = TRUE, encoding = "") {
    if (!is.null(x) & !is.list(x)) 
        stop("Dictionaries must be named lists or lists of named lists.")
    if (any(missingLabels <- which(names(x) == ""))) 
        stop("missing key name for list element", ifelse(length(missingLabels) > 1, "s ", " "), missingLabels, "\n")
    ...
    if (!is.null(file)) {
        ...
        if (format == "wordstat") 
            x <- readWStatDict(file, enc = encoding, toLower = toLower) else if (format == "LIWC") 
            x <- readLIWCdict(file, toLower = toLower, encoding = encoding) else if (format == "yoshikoder") 
            x <- readYKdict(file) else if (format == "lexicoder") 
...
# Import a Wordstat dictionary Make a flattened list from a hierarchical wordstat dictionary @param path full pathname of the wordstat dictionary file (usually ending in .cat) @param enc a valid input encoding for the file to be read, see \link{iconvlist} @param lower if \code{TRUE}
# (default), convert the dictionary entries to lower case @return a named list, where each the name of element is a bottom level category in the hierarchical wordstat dictionary. Each element is a list of the dictionary terms corresponding to that level.  @author Kohei Watanabe, Kenneth
# Benoit @export @examples \dontrun{ path <- '~/Dropbox/QUANTESS/corpora/LaverGarry.cat' lgdict <- readWStatDict(path) }
readWStatDict <- function(path, enc = "", toLower = TRUE) {
    ...
    # this loop collapses the category cells together and makes the list of named lists compatible with dfm
    for (i in 1:nrow(d)) {
        if (d[i, ncol(d)] == "") 
            next
...
# Import a LIWC-formatted dictionary Make a flattened dictionary list object from a LIWC dictionary file.  @param path full pathname of the LIWC-formatted dictionary file (usually a file ending in .dic) @param enc a valid input encoding for the file to be read, see \link{iconvlist} @param
# maxcats the maximum number of categories to read in, set by the maximum number of dictionary categories that a term could belong to.  For non-exclusive schemes such as the LIWC, this can be up to 7.  Set to 10 by default, which ought to be more than enough.  @return a dictionary class
# named list, where each the name of element is a bottom level category in the hierarchical wordstat dictionary. Each element is a list of the dictionary terms corresponding to that level.  @author Kenneth Benoit @export
readLIWCdict <- function(path, toLower = TRUE, encoding = getOption("encoding")) {
    ...
    oflines <- grep("<of>", d)
    if (length(oflines)) {
        catm("note: ", length(oflines), " term", ifelse(length(oflines) > 1, "s", ""), " ignored because contains unsupported <of> tag\n", sep = "")
        d <- d[-oflines]
    }
    ...
    # guide guide <- data.frame(do.call(rbind, strsplit(guide, '\t')), stringsAsFactors = FALSE)
    ...
    if (length(foundParens)) {
        ...
        for (i in foundParens) catm("  [line ", foundParens + guideRowEnd, ":] ", catlist[i], "\n", sep = "")
        catlist <- gsub("\\(.+\\)", "", catlist)
    }
    
    ## clean up irregular dictionary files remove any repeated \t
    catlist <- gsub("\t\t+", "\t", catlist)
    ...
    if (toLower) 
        catlist$category <- toLower(catlist$category)
...
# Import a Yoshikoder dictionary Make a flattened list from a hierarchical Yoshikoder dictionary.  Parsing Yoshikoder dictionary requires the XML package to be installed.  @param path full pathname of the Yoshikoder dictionary file (ending in \code{.ykd}) @return a named list, where each
# the name of element is a \textit{top} level category in the hierarchical Yoshikoder dictionary. Each element of the list is is a vector of the dictionary patterns in that category or any of its sub-categories.  @author Will Lowe @export @examples \dontrun{ path <-
# 'http://dl.conjugateprior.org/laver-garry-ajps.ykd' ykdict <- readYoshikoderDict(path) }
readYKdict <- function(path) {
    ...
    catnames <- XML::xpathSApply(xx, "/dictionary/cnode/cnode", XML::xmlGetAttr, name = "name")
...
flatten.dictionary <- function(elms, parent = "", dict = list()) {
    ...
    for (self in names(elms)) {
        ...
        # print('-------------------') print (paste('Name', self))
        if (is.list(elm)) {
            # print('List:') print(names(elm))
...
# Import a Lexicoder dictionary @param path full pathname of the lexicoder dictionary file (usually ending in .lcd) @param toLower if \code{TRUE} (default), convert the dictionary entries to lower case @return a named list, where each the name of element is a category/key and each element
# is a list of the dictionary terms corresponding to that level.  @author Adam Obeng @export
...
applyDictionary.dfm <- function(x, dictionary, exclusive = TRUE, valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, capkeys = !exclusive, verbose = TRUE, ...) {
    ...
    if (verbose) 
        catm("applying a dictionary consisting of ", length(dictionary), " key", ifelse(length(dictionary) > 1, "s", ""), "\n", sep = "")
    ...
    newFeatureIndexList <- lapply(dictionary, function(x) {
        ...
        if (valuetype == "fixed") {
            if (case_insensitive) 
                ind <- which(toLower(uniqueFeatures) %in% (toLower(x))) else ind <- which(uniqueFeatures %in% x)
        } else ind <- which(stringi::stri_detect_regex(uniqueFeatures, paste(x, collapse = "|"), case_insensitive = case_insensitive))
        if (length(ind) == 0) 
            return(NULL) else return(ind)
    })
    if (capkeys) 
        newFeatures <- stringi::stri_trans_toupper(newFeatures)
    newFeatureCountsList <- lapply(newFeatureIndexList, function(i) {
        if (!is.null(i)) 
            rowSums(x[, i]) else rep(0, nrow(x))
    })
    dfmresult2 <- new("dfmSparse", sparseMatrix(i = newDocIndex, j = rep(1:length(dictionary), each = ndoc(x)), x = unlist(newFeatureCountsList), dimnames = list(docs = docnames(x), features = newFeatures)))
    if (!exclusive) {
        if (!all(is.null(keyIndex <- unlist(newFeatureIndexList, use.names = FALSE)))) 
            dfmresult2 <- cbind(x[, -keyIndex], dfmresult2) else dfmresult2 <- cbind(x, dfmresult2)
    }
