...
#' @export
setClass("corpusSource", slots = c(texts = "character", docvars = "data.frame", source = "character", created = "character", cachedfile = "character"), prototype = list(cachedfile = ""))


#' @rdname corpusSource-class
#' @param object corpusSource object to be printed
#' @export
setMethod("show", signature(object = "corpusSource"), function(object) {
    if (object@cachedfile != "") {
        cat("corpusSource object with data cached in", object@cachedfile, "\n")
    } else {
        cat("corpusSource object consisting of ", length(texts(object)), " document", ifelse(length(texts(object)) == 1, "", "s"), " and ", ncol(docvars(object)), " docvar", ifelse(ncol(docvars(object)) == 1, "", "s"), ".\n", sep = "")
    }
})
...
setGeneric("textfile", function(file, ignoreMissingFiles = FALSE, textField = NULL, cache = FALSE, docvarsfrom = c("filenames"), dvsep = "_", docvarnames = NULL, encoding = NULL, ...) standardGeneric("textfile"))
# signature = c('file', 'textField', 'encodingFrom', 'encodingTo', 'docvarsfrom', 'dvsep', 'docvarnames', 'cache', 'encodingFrom', 'encodingTo'))
...
#' }
setMethod("textfile", signature(file = "character", ignoreMissingFiles = "ANY", textField = "ANY", cache = "ANY", docvarsfrom = "ANY", dvsep = "ANY", docvarnames = "ANY", encoding = "ANY"), definition = function(file, ignoreMissingFiles = FALSE, textField = NULL, cache = FALSE, docvarsfrom = "metadata", 
    dvsep = "_", docvarnames = NULL, encoding = NULL, ...) {
    
    if (is.null(textField)) 
        textField <- 1
    ...
    returnCorpusSource(list(txts = unlist(lapply(sources, function(x) x$txts)), docvars = data.frame(docvars)), cache)
})
...
listMatchingFiles <- function(x, ignoreMissing = F, lastRound = F) {
    # The implementation of listMatchingFiles and listMatchingFile might seem very complex, but it was arrived at after a lot of toil. The main design decision made here is that the user should be able to pass many different types of string to listMatchingFiles and get a consistent result: a
    # list of local filenames. (One additional wrinkle is that there are two functions, listMatchingFiles and listMatchingFile. This is to ensure that listMatchingFile is only ever called with a length 1 argument, even though it can return multiple filenames. For the purposes of this
    # explanation, this distinction is elided).  There are four possible types of values for x - a simple filename - a remote URL - a glob pattern - a vector of some combination of the above listMatchingFiles has a recursive design, because some of these arguments can resolve to arguments which
    # need further processing: e.g.  a remote URL could resolve to a zip file which needs to be extracted.  The termination condition for the recursion is when the argument passed is a local filepath which refers to a single file and needs no further processing, e.g. something like
    # '/path/to/text.tsv'. However, it is not possible to determine if a given argument is a path to a single file or a glob pattern which matches multiple files, without actually trying the match. This matters because if it's the result of a globbing expression, then it could potentially need
    # further processing, but if it's not, it the recursion needs to end. We can't know beforehand because the rules for globbing are implementation-dependent (systems might treat '/path/to/file\*.tsv' as either a filename or a path depending on whether they support escaping of glob wildcards.
    # We could have tested the return value from Sys.glob to see whether the system treats a given string as a glob pattern or a simple filename. Unfortunately, Sys.glob() will return character(0) for either a glob pattern which matches no files, or a non-glob filename for a file that doesn't
    # exist, so that doesn't work either.  We also can't test whether a pattern is a regular file by looking at the extension, because '/path/to/*.zip' is a glob expression with a 'zip' extension.
    
    if (!(ignoreMissing || (length(x) > 0))) {
        stop("File does not exist.")
    }
    
    matchingFiles <- unlist(lapply(x, function(x) listMatchingFile(x, ignoreMissing = ignoreMissing, lastRound = lastRound)))
    
    if (is.null(matchingFiles)) 
        return(character(0))
...
extractArchive <- function(i, ignoreMissing) {
    ...
    if (tools::file_ext(i) == "zip") 
        utils::unzip(i, exdir = td) else if (tools::file_ext(i) == "gz" || tools::file_ext(i) == "tar" || tools::file_ext(i) == "bz") 
...
listMatchingFile <- function(x, ignoreMissing, verbose = F, lastRound) {
    ...
    if (tools::file_ext(i) == "zip" || tools::file_ext(i) == "gz" || tools::file_ext(i) == "tar" || tools::file_ext(i) == "bz") {
        if (verbose) 
            message("archive")
    ...
    # At this point, it may be a simple local file or a glob pattern, but as above, we have no way of telling a priori whether this is the case
    if (lastRound) {
        # We get to this point if the path wasn't to some file that needed special treatment (zip, remote, etc.) and it was treated as a glob pattern, which means that it is definitely not a glob pattern this time
        if (!(ignoreMissing || file.exists(i))) 
        ...
        if (verbose) 
            message("possible glob pattern")
        i <- Sys.glob(i)
        return(listMatchingFiles(i, ignoreMissing = ignoreMissing, lastRound = T))
    }
    
}


# function common to all textfile methods to return either the cached textfile object link, or the textfile object itself
returnCorpusSource <- function(sources, cache = FALSE) {
    ...
    } else return(new("corpusSource", texts = sources$txts, docvars = imputeDocvarsTypes(sources$docv)))
...
getSource <- function(f, textField, ...) {
    ...
    newSource <- switch(fileType, txt = get_txt(f, ...), csv = get_csv(f, textField, sep = ",", ...), tsv = get_csv(f, textField, sep = "\t", ...), json = get_json(f, textField, ...), xml = get_xml(f, textField, ...))
...
# Dispatch to get_json_object or get_json_tweets depending on whether it looks like a twitter json file
...
get_json_object <- function(path, textField, ...) {
    ...
    list(txts = docs[[textField]], docv = docs[, -textField, with = F])
}
...
get_json_lines <- function(path, textField, ...) {
    ...
    docs <- data.table::rbindlist(lapply(lines, function(x) jsonlite::fromJSON(x, flatten = TRUE, ...)), use.names = TRUE, fill = TRUE)
...
get_xml <- function(path, textField, encoding, ...) {
    ...
    if (is.character(textField)) {
        ...
        warning(paste("You should specify textField by name rather than by index, unless", "you're certain that your XML file's fields are always in the same order."))
    }
    
    txts <- docs[, textField]
    docv <- docs[, -textField, drop = FALSE]
    
    # Because XML::xmlToDataFrame doesn't impute column types, we have to do it ourselves, to match get_csv's behaviour
    list(txts = txts, docv = docv)
}
...
getdocvarsFromFilenames <- function(fnames, dvsep = "_", docvarnames = NULL) {
    ...
    dvars <- data.frame(matrix(unlist(parts), nrow = length(parts), byrow = TRUE), stringsAsFactors = FALSE)
    ...
    if (!is.null(docvarnames)) {
        ...
        if (length(docvarnames) != ncol(dvars)) {
            warning("Fewer docnames supplied than existing docvars - last ", ncol(dvars) - length(docvarnames), " docvar", ifelse((ncol(dvars) - length(docvarnames)) == 1, "", "s"), " given generic names.")
        }
    }
    dvars
}
