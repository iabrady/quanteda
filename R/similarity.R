...
#' }
#' @export
setGeneric("similarity", signature = c("x", "selection", "n", "margin", "sorted", "normalize"), def = function(x, selection = NULL, n = NULL, margin = c("documents", "features"), method = "correlation", sorted = TRUE, normalize = FALSE) standardGeneric("similarity"))

#' @rdname similarity
#' @export
setMethod("similarity", signature = signature("dfm", "ANY"), def = function(x, selection = NULL, n = NULL, margin = c("documents", "features"), method = "correlation", sorted = TRUE, normalize = FALSE) {
    ...
    if (method == "cosine") {
        ...
        similmatrix <- as.matrix(proxy::simil(as.matrix(x), as.matrix(xSelect), method = method, by_rows = ifelse(margin == "features", FALSE, TRUE)), diag = 1)
    }
    
    # rounding if (!is.null(digits)) similmatrix <- round(similmatrix, digits)
    
    # convert NaNs to NA similmatrix[is.nan(similmatrix)] <- NA
    
    # return the matrix if this was requested return(similmatrix)
    
    # convert the matrix to a list of similarities
    result <- lapply(seq_len(ncol(similmatrix)), function(i) similmatrix[, i])
    names(result) <- if (!is.null(xSelect)) 
        items[selectIndex] else if (margin == "documents") 
        docnames(x) else features(x)
    
    # remove the element of each similarity vector equal to the item itself
    tempseq <- seq_along(result)
    names(tempseq) <- names(result)
    result <- lapply(tempseq, function(i) result[[i]] <- result[[i]][-which(names(result[[i]]) == names(result)[i])])
    ...
    # return a vector if list of length 1, otherwise return a named list if (length(result)==1) result <- result[[1]]
...
as.matrix.similMatrix <- function(x, ...) {
    ...
    tmpdf <- do.call(rbind, lapply(lapply(x, unlist), "[", unique(unlist(c(sapply(x, names))))))
...
## code below based on assoc.R from the qlcMatrix package used Matrix::crossprod and Matrix::tcrossprod for sparse Matrix handling

# L2 norm
norm2 <- function(x, s) {
    drop(Matrix::crossprod(x^2, s))^0.5
}
# L1 norm
norm1 <- function(x, s) {
    drop(Matrix::crossprod(abs(x), s))
}

cosineSparse <- function(x, y = NULL, margin = 1, norm = norm2) {
    if (!(margin %in% 1:2)) 
        stop("margin can only be 1 (rows) or 2 (columns)")
    if (margin == 1) 
        x <- t(x)
    S <- rep(1, nrow(x))
    ...
    } else return(as.matrix(Matrix::crossprod(x)))
}
...
correlationSparse <- function(x, y = NULL, margin = 1) {
    if (!(margin %in% 1:2)) 
        stop("margin can only be 1 (rows) or 2 (columns)")
    cpFun <- if (margin == 2) 
        Matrix::crossprod else Matrix::tcrossprod
    tcpFun <- if (margin == 2) 
        Matrix::tcrossprod else Matrix::crossprod
    marginSums <- if (margin == 2) 
        colSums else rowSums
    
    n <- if (margin == 2) 
        nrow(x) else ncol(x)
    muX <- if (margin == 2) 
        colMeans(x) else rowMeans(x)
    
    if (!is.null(y)) {
        stopifnot(ifelse(margin == 2, nrow(x) == nrow(y), ncol(x) == ncol(y)))
        muY <- if (margin == 2) 
            colMeans(y) else rowMeans(y)
