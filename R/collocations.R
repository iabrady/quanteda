...
collocations <- function(x, ...) {
    # addedArgs <- names(list(...)) if (length(addedArgs) && any(!(addedArgs %in% names(formals(getS3method('tokenize', 'character')))))) warning('Argument', ifelse(length(addedArgs)>1, 's ', ' '), addedArgs, ' not used.', sep = '', noBreaks. = TRUE)
...
collocations3 <- function(x, method = c("lr", "chi2", "pmi", "dice", "all"), size = 3, n = NULL, ...) {
    ...
    wordpairs <- data.table(w1 = t[1:(length(t) - 2)], w2 = t[2:(length(t) - 1)], w3 = t[3:(length(t))], count = 1)
    
    # eliminate non-adjacent words
    wordpairs <- wordpairs[!(stri_detect_regex(w1, "^\\p{P}$") | stri_detect_regex(w2, "^\\p{P}$") | stri_detect_regex(w3, "^\\p{P}$"))]
    
    # set the data.table sort key
    setkey(wordpairs, w1, w2, w3)
    
    ## counts of trigrams and bigrams tabulate (count) w1 w2 pairs
    ...
    # # eliminate any duplicates in w3 - see note above in collocations2 dups <- which(duplicated(w12Table[, w1, w2])) if (length(dups)) { catm(' ...NOTE: dropping duplicates in word1,2: ... \n', w12Table[dups, w1, w2], '\n') w3Table <- w3Table[-dups] }
    ...
    # ## testing from McInnes thesis Tables 19-20 example allTable <- rbind(allTable, allTable[478,]) allTable$n111[479] <- 171 allTable$n112[479] <- 3000 allTable$n121[479] <- 2 allTable$n122[479] <- 20805 allTable$n211[479] <- 4 allTable$n212[479] <- 2522 allTable$n221[479] <- 7157
    # allTable$n222[479] <- 88567875 N <- 88601536
    
    # expected counts m_{ijk} for first independence model
    epsilon <- 1e-09  # to offset zero cell counts
    allTable <- within(allTable, {
        ...
        # 'Model 2': P(w1,w2,w3) = P(w1,w2)P(w3) m2.111 <- exp(log(n111 + n112) + log(n111 + n211 + n121 + n221) - log(N)) m2.112 <- exp(log(n111 + n112) + log(n112 + n212 + n122 + n222) - log(N)) m2.121 <- exp(log(n121 + n122) + log(n111 + n211 + n121 + n221) - log(N)) m2.122 <- exp(log(n121 +
        # n122) + log(n112 + n212 + n122 + n222) - log(N)) m2.211 <- exp(log(n211 + n212) + log(n111 + n211 + n121 + n221) - log(N)) m2.212 <- exp(log(n211 + n212) + log(n112 + n212 + n122 + n222) - log(N)) m2.221 <- exp(log(n221 + n222) + log(n111 + n211 + n121 + n221) - log(N)) m2.222 <-
        # exp(log(n221 + n222) + log(n112 + n212 + n122 + n222) - log(N)) # 'Model 2': P(w1,w2,w3) = P(w1)P(w2,w3) m3.111 <- exp(log(n111 + n211) + log(n111 + n121 + n112 + n122) - log(N)) m3.112 <- exp(log(n112 + n212) + log(n111 + n121 + n112 + n122) - log(N)) m3.121 <- exp(log(n121 + n221) +
        # log(n111 + n121 + n112 + n122) - log(N)) m3.122 <- exp(log(n122 + n222) + log(n111 + n121 + n112 + n122) - log(N)) m3.211 <- exp(log(n111 + n211) + log(n211 + n221 + n212 + n222) - log(N)) m3.212 <- exp(log(n112 + n212) + log(n211 + n221 + n212 + n222) - log(N)) m3.221 <- exp(log(n121 +
        # n221) + log(n211 + n221 + n212 + n222) - log(N)) m3.222 <- exp(log(n122 + n222) + log(n211 + n221 + n212 + n222) - log(N))
        
        lrratioM1 <- 2 * ((n111 * log(n111/m1.111 + epsilon)) + (n112 * log(n112/m1.112 + epsilon)) + (n121 * log(n121/m1.121 + epsilon)) + (n122 * log(n122/m1.122 + epsilon)) + (n211 * log(n211/m1.211 + epsilon)) + (n212 * log(n212/m1.212 + epsilon)) + (n221 * log(n221/m1.221 + epsilon)) + 
            (n222 * log(n222/m1.222 + epsilon)))
        # lrratioM2 <- 2 * ((n111 * log((n111 + epsilon) / (m2.111 + epsilon))) + (n112 * log((n112 + epsilon) / (m2.112 + epsilon))) + (n121 * log((n121 + epsilon) / (m2.121 + epsilon))) + (n122 * log((n122 + epsilon) / (m2.122 + epsilon))) + (n211 * log((n211 + epsilon) / (m2.211 + epsilon))) +
        # (n212 * log((n212 + epsilon) / (m2.212 + epsilon))) + (n221 * log((n221 + epsilon) / (m2.221 + epsilon))) + (n222 * log((n222 + epsilon) / (m2.222 + epsilon)))) lrratioM3 <- 2 * ((n111 * log((n111 + epsilon) / (m3.111 + epsilon))) + (n112 * log((n112 + epsilon) / (m3.112 + epsilon))) +
        # (n121 * log((n121 + epsilon) / (m3.121 + epsilon))) + (n122 * log((n122 + epsilon) / (m3.122 + epsilon))) + (n211 * log((n211 + epsilon) / (m3.211 + epsilon))) + (n212 * log((n212 + epsilon) / (m3.212 + epsilon))) + (n221 * log((n221 + epsilon) / (m3.221 + epsilon))) + (n222 * log((n222 +
        # epsilon) / (m3.222 + epsilon))))
        
    })
    
    allTable <- within(allTable, {
        chi2 <- ((n111 - m1.111)^2/m1.111) + ((n112 - m1.112)^2/m1.112) + ((n121 - m1.121)^2/m1.121) + ((n122 - m1.122)^2/m1.122) + ((n211 - m1.211)^2/m1.211) + ((n212 - m1.212)^2/m1.212) + ((n221 - m1.221)^2/m1.221) + ((n222 - m1.222)^2/m1.222)
        pmi <- log(n111/m1.111)
        dice <- 2 * n111/(n111 + n121 + n112 + n122 + n111 + n211 + n112 + n212 + n111 + n211 + n121 + n221)
    })
    
    dt <- data.table(word1 = allTable$w1, word2 = allTable$w2, word3 = allTable$w3, count = allTable$c123)
    ...
    if (method == "chi2") {
        ...
        # dt$G2m1 <- allTable$lrratioM1 dt$G2m2 <- allTable$lrratioM2 dt$G2m3 <- allTable$lrratioM3 setorder(dt, -G2m1)
...
#' @rdname collocations
#' @export    
collocations.character <- function(x, method = c("lr", "chi2", "pmi", "dice", "all"), size = 2, n = NULL, spanPunct = FALSE, ...) {
    method <- match.arg(method)
    # 'Enough is enough! I have had it with these motherfucking snakes on this motherfucking plane!' x <- iconv(x, 'UTF-8', 'ASCII', sub='') # opening some windows tagset <- match.arg(tagset)
...
collocations.tokenizedTexts <- function(x, method = c("lr", "chi2", "pmi", "dice", "all"), size = 2, n = NULL, spanPunct = FALSE, ...) {
    ...
    if (3 %in% size) {
        if (is.null(coll)) 
            coll <- collocations3(x, method, 3, n, ...) else {
...
collocations2 <- function(x, method = c("lr", "chi2", "pmi", "dice", "all"), size = 2, n = NULL, spanPunct = FALSE, ...) {
    ...
    t <- x
    # t <- factor(x) tlevels <- levels(t) t <- as.integer(t)
    
    # create a data.table of all adjacent bigrams
    wordpairs <- data.table(w1 = t[1:(length(t) - 1)], w2 = t[2:length(t)], count = 1)
    
    # eliminate non-adjacent words (where a blank is in a pair)
    if (!spanPunct) {
        wordpairs <- wordpairs[!(stringi::stri_detect_regex(w1, "^[\\p{P}\\p{S}]$") | stringi::stri_detect_regex(w2, "^[\\p{P}\\p{S}]$"))]
    }
    ...
    # eliminate any duplicates in w1 - although this ought not to happen!  bug in data.table??  encoding problem on our end??
    dups <- which(duplicated(w1Table[, w1]))
    if (length(dups)) {
        catm("  ...NOTE: dropping duplicates in word1:", w1Table[dups, w1], "\n")
        w1Table <- w1Table[-dups]
    ...
    if (method == "all" | method == "lr") {
        allTable2$lrratio <- 2 * ((allTable2$w1w2n * log(allTable2$w1w2n/(allTable2$w1w2Exp + epsilon) + epsilon)) + (allTable2$w1notw2 * log(allTable2$w1notw2/(allTable2$w1notw2Exp + epsilon) + epsilon)) + (allTable2$notw1w2 * log(allTable2$notw1w2/(allTable2$notw1w2Exp + epsilon) + epsilon)) + 
            (allTable2$notw1notw2 * log(allTable2$notw1notw2/(allTable2$notw1notw2Exp + epsilon) + epsilon)))
    }
    if (method == "all" | method == "chi2") {
        allTable2$chi2 <- (allTable2$w1w2n - allTable2$w1w2Exp)^2/allTable2$w1w2Exp + (allTable2$w1notw2 - allTable2$w1notw2Exp)^2/allTable2$w1notw2Exp + (allTable2$notw1w2 - allTable2$notw1w2Exp)^2/allTable2$notw1w2Exp + (allTable2$notw1notw2 - allTable2$notw1notw2Exp)^2/allTable2$notw1notw2Exp
    }
    ...
    if (method == "chi2") {
        ...
        df <- data.table(word1 = allTable2$w1, word2 = allTable2$w2, word3 = "", count = allTable2$w1w2n, X2 = allTable2$chi2)
    } else if (method == "pmi") {
        setorder(allTable2, -pmi)
        df <- data.table(word1 = allTable2$w1, word2 = allTable2$w2, word3 = "", count = allTable2$w1w2n, pmi = allTable2$pmi)
        
    } else if (method == "dice") {
        setorder(allTable2, -dice)
        df <- data.table(word1 = allTable2$w1, word2 = allTable2$w2, word3 = "", count = allTable2$w1w2n, dice = allTable2$dice)
    } else {
        setorder(allTable2, -lrratio)
        df <- data.table(word1 = allTable2$w1, word2 = allTable2$w2, word3 = "", count = allTable2$w1w2n, G2 = allTable2$lrratio)
    }
    ...
    # df[, word1 := factor(word1, levels = 1:length(tlevels), labels = tlevels)] df[, word2 := factor(word2, levels = 1:length(tlevels), labels = tlevels)]
