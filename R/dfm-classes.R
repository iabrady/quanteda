#################################################################### dfm class definition and methods for primitives, ops, etc.  Ken Benoit
...
#' @docType class
#' @name dfm-class
setClass("dfm", slots = c(settings = "list", weightTf = "list", weightDf = "list", smooth = "numeric", ngrams = "integer", concatenator = "character"), prototype = list(settings = list(NULL), Dim = integer(2), Dimnames = list(docs = NULL, features = NULL), weightTf = list(scheme = "count", base = NULL, 
    K = NULL), weightDf = list(scheme = "unary", base = NULL, c = NULL, smoothing = NULL, threshold = NULL), smooth = 0, ngrams = 1L, concatenator = ""), contains = "Matrix")
...
#'   per cell.)
#' @export
setClass("dfmSparse", contains = c("dfm", "dgCMatrix"))
...
#' @export
setClass("dfmDense", contains = c("dfm", "dgeMatrix"))



# # @rdname print.dfm # @export setMethod('print', signature(x = 'dfm'), callNextMethod())
...
#' @export
#' @rdname head.dfm
setMethod("head", signature(x = "dfm"), function(x, n = 6L, nfeature = 6L, ...) head.dfm(x, n = n, nfeature = nfeature, ...))

#' @export
#' @rdname head.dfm
setMethod("tail", signature(x = "dfm"), function(x, n = 6L, nfeature = 6L, ...) tail.dfm(x, n = n, nfeature = nfeature, ...))
...
#' @export
#' @rdname print.dfm
setMethod("print", signature(x = "dfm"), function(x, show.values = FALSE, show.settings = FALSE, show.summary = TRUE, ndoc = 20L, nfeature = 20L, ...) {
    if (show.summary) {
        cat("Document-feature matrix of: ", format(ndoc(x), , big.mark = ","), " document", ifelse(ndoc(x) > 1 | ndoc(x) == 0, "s, ", ", "), format(nfeature(x), big.mark = ","), " feature", ifelse(nfeature(x) > 1 | nfeature(x) == 0, "s", ""), ifelse(is.resampled(x), paste(", ", nresample(x), 
            " resamples", sep = ""), ""), ".\n", sep = "")
    }
    if (show.settings) {
    ...
    if (show.values | (nrow(x) <= ndoc & ncol(x) <= nfeature)) {
        Matrix::printSpMatrix2(x[1:min(ndoc, ndoc(x)), 1:min(nfeature, nfeature(x))], col.names = TRUE, zero.print = 0, ...)
    }
})
...
#' @export
#' @rdname print.dfm
setMethod("print", signature(x = "dfmSparse"), function(x, show.values = FALSE, show.settings = FALSE, show.summary = TRUE, ndoc = 20L, nfeature = 20L, ...) {
    if (show.summary) {
        cat("Document-feature matrix of: ", format(ndoc(x), , big.mark = ","), " document", ifelse(ndoc(x) > 1 | ndoc(x) == 0, "s, ", ", "), format(nfeature(x), big.mark = ","), " feature", ifelse(nfeature(x) > 1 | nfeature(x) == 0, "s", ""), ifelse(is.resampled(x), paste(", ", nresample(x), 
            " resamples", sep = ""), ""), ".\n", sep = "")
    }
...
#' @rdname print.dfm
setMethod("print", signature(x = "dfmDense"), function(x, show.values = FALSE, show.settings = FALSE, show.summary = TRUE, ndoc = 20L, nfeature = 20L, ...) {
    if (show.summary) {
        cat("Document-feature matrix of: ", format(ndoc(x), , big.mark = ","), " document", ifelse(ndoc(x) > 1 | ndoc(x) == 0, "s, ", ", "), format(nfeature(x), big.mark = ","), " feature", ifelse(nfeature(x) > 1 | nfeature(x) == 0, "s", ""), ifelse(is.resampled(x), paste(", ", nresample(x), 
            " resamples", sep = ""), ""), ".\n", sep = "")
    }
...
print.dfm <- function(x, show.values = FALSE, show.settings = FALSE, show.summary = TRUE, ndoc = 20L, nfeature = 20L, ...) {
    if (show.summary) {
        cat("Document-feature matrix of: ", ndoc(x), " document", ifelse(ndoc(x) > 1, "s, ", ", "), dim(x)[2], " feature", ifelse(dim(x)[2] > 1, "s", ""), ".\n", sep = "")
    }
...
## S4 Method for the S4 class sparse dfm @param x the sparse dfm @rdname dfm-class @method t dfmSparse setMethod('t', signature(x = 'dfmSparse'), getMethod('t', 'dgCMatrix'))

# S4 Method for the S4 class dense/weighted dfm
#' @export
#' @rdname dfm-class
setMethod("t", signature(x = "dfmDense"), definition = function(x) {
...
#' @rdname dfm-class 
setMethod("t", signature = (x = "dfmSparse"), definition = function(x) {
...
#' @param dims ignored
#' @export
setMethod("colSums", signature = (x = "dfmSparse"), function(x, na.rm = FALSE, dims = 1L, ...) callNextMethod())

#' @method rowSums dfmSparse
#' @rdname dfm-class
#' @export
setMethod("rowSums", signature = (x = "dfmSparse"), function(x, na.rm = FALSE, dims = 1L, ...) callNextMethod())


#' @method colMeans dfmSparse
#' @rdname dfm-class
#' @export
setMethod("colMeans", signature = (x = "dfmSparse"), function(x, na.rm = FALSE, dims = 1L, ...) callNextMethod())

#' @method rowSums dfmSparse
#' @rdname dfm-class
#' @export
setMethod("rowMeans", signature = (x = "dfmSparse"), function(x, na.rm = FALSE, dims = 1L, ...) callNextMethod())



# #' @param i index for documents #' @param j index for features #' @param drop always set to \code{FALSE} #' @param ... additional arguments not used here #' @export #' @rdname dfm-class setMethod('[', signature = c(x = 'dfmSparse', i = 'ANY', j = 'ANY', drop = 'logical'), function(x, i,
# j, ..., drop = FALSE) { cat('XXXXXX\n') new('dfmSparse', getMethod('[', 'dgCMatrix')(x, i, j, ..., drop = FALSE)) }) #' @rdname dfm-class #' @export setMethod('[', signature = (x = 'dfmDense'), function(x, i, j, ..., drop = FALSE) { cat('OH YEAH\n') new('dfmDense', callNextMethod()) })

## S4 METHODS FOR INDEXING SPARSE dfm (dfmSparse) objects


# FROM THE MATRIX PACKAGE - no need to duplicate here setClassUnion('index', members = c('numeric', 'integer', 'logical', 'character'))
...
wrapIndexOperation <- function(x, i = NULL, j = NULL, ..., drop = FALSE) {
    ...
    if (drop) 
        warning("drop=TRUE not currently supported")
...
#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "index", j = "index", drop = "missing"), wrapIndexOperation)

#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "index", j = "index", drop = "logical"), wrapIndexOperation)

#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "index", j = "missing", drop = "missing"), function(x, i, j, ..., drop = FALSE) new("dfmDense", as(x, "denseMatrix")[i, , ..., drop = FALSE]))

#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "logical", j = "missing", drop = "missing"), function(x, i, j, ..., drop = FALSE) new("dfmDense", as(x, "denseMatrix")[which(i), , ..., drop = FALSE]))

#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "index", j = "missing", drop = "logical"), function(x, i, j, ..., drop = FALSE) {
    if (drop) 
        warning("drop=TRUE not currently supported")
...
#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "missing", j = "index", drop = "missing"), function(x, i, j, ..., drop = FALSE) new("dfmDense", as(x, "denseMatrix")[, j, ..., drop = FALSE]))

#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "missing", j = "index", drop = "logical"), function(x, i, j, ..., drop = FALSE) {
    if (drop) 
        warning("drop=TRUE not currently supported")
...
#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "missing", j = "missing", drop = "missing"), function(x, i, j, ..., drop = FALSE) new("dfmDense", as(x, "denseMatrix")[, , ..., drop = FALSE]))

#' @rdname dfm-class
setMethod("[", signature(x = "dfmDense", i = "missing", j = "missing", drop = "logical"), function(x, i, j, ..., drop = FALSE) {
    if (drop) 
        warning("drop=TRUE not currently supported")
...
#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "index", j = "index", drop = "missing"), wrapIndexOperation)

#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "index", j = "index", drop = "logical"), wrapIndexOperation)


#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "logical", j = "missing", drop = "missing"), function(x, i, j, ..., drop = FALSE) {
    new("dfmSparse", as(x, "sparseMatrix")[which(i), , ..., drop = FALSE])
})

#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "index", j = "missing", drop = "missing"), function(x, i, j, ..., drop = FALSE) {
    new("dfmSparse", as(x, "sparseMatrix")[i, , ..., drop = FALSE])
})

#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "index", j = "missing", drop = "logical"), function(x, i, j, ..., drop = FALSE) {
    if (drop) 
        warning("drop=TRUE not currently supported")
    new("dfmSparse", as(x, "sparseMatrix")[i, , ..., drop = FALSE])
})
...
#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "missing", j = "index", drop = "missing"), function(x, i, j, ..., drop = FALSE) new("dfmSparse", as(x, "sparseMatrix")[, j, ..., drop = FALSE]))

#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "missing", j = "index", drop = "logical"), function(x, i, j, ..., drop = FALSE) {
    if (drop) 
        warning("drop=TRUE not currently supported")
...
#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "missing", j = "missing", drop = "missing"), function(x, i, j, ..., drop = FALSE) new("dfmSparse", as(x, "sparseMatrix")[, , ..., drop = FALSE]))

#' @rdname dfm-class
setMethod("[", signature(x = "dfmSparse", i = "missing", j = "missing", drop = "logical"), function(x, i, j, ..., drop = FALSE) {
    if (drop) 
        warning("drop=TRUE not currently supported")
...
#' @rdname dfm-class
setMethod("+", signature(e1 = "dfmSparse", e2 = "numeric"), function(e1, e2) {
    as(as(e1, "Matrix") + e2, ifelse(e2 == 0, "dfmSparse", "dfmDense"))
})
#' @rdname dfm-class
setMethod("+", signature(e1 = "numeric", e2 = "dfmSparse"), function(e1, e2) {
    as(e1 + as(e2, "Matrix"), ifelse(e1 == 0, "dfmSparse", "dfmDense"))
})
#' @rdname dfm-class
setMethod("+", signature(e1 = "dfmDense", e2 = "numeric"), function(e1, e2) {
    as(as(e1, "Matrix") + e2, "dfmDense")
})
#' @rdname dfm-class
setMethod("+", signature(e1 = "numeric", e2 = "dfmDense"), function(e1, e2) {
...
setMethod("as.matrix", signature(x = "dfm"), function(x) {
    if (isS4(x)) {
...
setMethod("as.data.frame", signature = "dfm", function(x, row.names = NULL, optional = FALSE, ...) {
...
cbind.dfm <- function(...) {
    ...
    if (length(args) > 2) {
        for (y in args[3:length(args)]) result <- Matrix::cbind2(result, y)
    }
    new("dfmSparse", result)
}

# setMethod('cbind', signature(x = 'dfmSparse', y = 'dfmSparse'), function(x, y, ...) { getMethod('cbind', 'dgCMatrix')(x, y, ...)  })
...
rbind.dfm <- function(...) {
    ...
    if (length(args) == 1) {
        ...
        for (y in args[3:length(args)]) result <- rbind2.dfm(result, y)
        return(result)
    }
}
...
rbind2.dfm <- function(x, y) {
    ...
    #   Evetually, we want to rbind together two rows with the same columns   we acheive this by re-ordering the columns in the original matrices, and adding in some blank ones
    
    # Here's what we're constructing: row x ... <columns in both x and y> ... <columns only in x> ... <blank columns> row y ... <columns in both x and y> ... <blank columns> ... <columns only in y>
    xcols <- Matrix::cbind2(x[, common_names], x[, only_x_names])
    empty_ycols <- Matrix::sparseMatrix(i = NULL, j = NULL, dims = c(ndoc(x), length(only_y_names)), dimnames = list(NULL, only_y_names))
    
    empty_xcols <- Matrix::sparseMatrix(i = NULL, j = NULL, dims = c(ndoc(y), length(only_x_names)), dimnames = list(NULL, only_x_names))
    
    y_with_xcols <- Matrix::cbind2(y[, common_names], empty_xcols)
    
    new_dfm <- new("dfmSparse", Matrix::rbind2(Matrix::cbind2(xcols, empty_ycols), Matrix::cbind2(y_with_xcols, y[, only_y_names])))
}
...
rbind2.dfm.old <- function(x, y) {
    ...
    addedx <- new("dfmSparse", Matrix::cbind2(x, sparseMatrix(i = NULL, j = NULL, dims = c(ndoc(x), length(toAddx)), dimnames = list(docnames(x), toAddx))))
    addedy <- new("dfmSparse", Matrix::cbind2(y, sparseMatrix(i = NULL, j = NULL, dims = c(ndoc(y), length(toAddy)), dimnames = list(docnames(y), toAddy))))
