...
selectFeaturesOLD.tokenizedTexts <- function(x, features, selection = c("keep", "remove"), valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, verbose = TRUE, ...) {
    ...
    if (valuetype == "fixed") {
        ...
        if (selection == "remove") {
            result <- lapply(x, function(y) {
                removeIndex <- which(stringi::stri_detect_regex(y, rep(paste0(features, collapse = "|"), length(y)), case_insensitive = case_insensitive, ...))
                if (length(removeIndex)) 
                  y[-removeIndex] else y
            })
        } else {
            result <- lapply(x, function(y) y[which(stringi::stri_detect_regex(y, rep(paste0(features, collapse = "|"), length(y)), case_insensitive = case_insensitive, ...))])
        }
    }
