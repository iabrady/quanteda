...
plot.kwic <- function(..., scale = c("absolute", "relative"), sort = FALSE) {
    ...
    ## edited by KB 29 May 2016 x <- lapply(kwics, function(i) { i$keyword <- attr(i, 'keyword'); i}) x <- lapply(kwics, function(i) { ntokens <- data.table::data.table( docname = names(attr(i, 'ntoken')), ntoken = attr(i, 'ntoken') ) merge(i, ntokens, all.x=TRUE, by='docname') }) x <-
    ## data.table::data.table(do.call(rbind, x))
    
    ## edited by KB 29 May 2016 create a data.table from the kwic arguments
    ...
    # pre-emptively convert keyword to factor before ggplot does it, so that we can keep the order of the factor the same as the order of the kwic objects
    x[, `:=`(keyword, factor(keyword, levels = unique(keyword)))]
    
    multiple_documents <- length(unique(x$docname)) > 1
    
    # Deal with the scale argument: if there is a user-supplied value, use that after passing through match.argj if not, use relative for multiple documents and absolute for single documents
    if (!missing(scale)) {
    ...
    plot <- ggplot2::ggplot(x, ggplot2::aes(x = position, y = 1)) + ggplot2::geom_segment(ggplot2::aes(xend = position, yend = 0)) + ggplot2::theme(axis.line = ggplot2::element_blank(), panel.background = ggplot2::element_blank(), panel.grid.major.y = ggplot2::element_blank(), panel.grid.minor.y = ggplot2::element_blank(), 
        plot.background = ggplot2::element_blank(), axis.ticks.y = ggplot2::element_blank(), axis.text.y = ggplot2::element_blank(), panel.margin = grid::unit(0.1, "lines"), panel.border = ggplot2::element_rect(colour = "gray", fill = NA), strip.text.y = ggplot2::element_text(angle = 0))
    
    if (scale == "absolute") 
        plot <- plot + ggplot2::geom_rect(ggplot2::aes(xmin = ntokens, xmax = max(x$ntokens), ymin = 0, ymax = 1), fill = "gray90")
    
    if (multiple_documents) {
        # If there is more than one document, put documents on the panel y-axis and keyword(s) on the panel x-axis
        plot <- plot + ggplot2::facet_grid(docname ~ keyword) + ggplot2::labs(y = "Document", title = paste("Lexical dispersion plot"))
    } else {
        # If not, put keywords on the panel y-axis and the document name in the title
        plot <- plot + ggplot2::facet_grid(keyword ~ .) + ggplot2::labs(y = "", title = paste("Lexical dispersion plot, document:", x$docname[[1]]))
    }
