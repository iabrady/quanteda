# @param object The texts to be described @param verbose set to \code{FALSE} to suppress printing, for instance if you simply want to assign the output to a \code{data.frame} @param ... additional arguments affecting the summary produced
...
summary.character <- function(object, n = 100, verbose = TRUE, toLower = FALSE, ...) {
    ...
    ntokens <- ntoken(toks)
    ntypes <- ntype(toks)
    results <- data.frame(Text = names(object), Types = ntypes, Tokens = ntokens, Sentences = nsents, row.names = NULL)
    if (verbose) 
        print(results, ...)
