### IN THIS FILE, THE MAIN METHOD and OBJECTS are S4.  This allows multiple dispatch, esp. for the S4 dfms, but also still works on the S3 old-style matrix-based dfm.  textmodel creates S4 objects now inheriting from textmodel_fitted-class the print, summary, predict methods are STILL S3 --
### which is why they do not dispatch properly unless print() (e.g.) is called specifically, but this could be easily solved by writing appropriate methods in S4.


#' @rdname textmodel_fitted-class 
#' @export
setClass("textmodel_wordscores_fitted", slots = c(scale = "character", Sw = "numeric"), prototype = list(scale = "linear"), contains = "textmodel_fitted")

#' @rdname textmodel_fitted-class
#' @export
setClass("textmodel_wordscores_predicted", slots = c(newdata = "dfm", rescaling = "character", level = "numeric", textscores = "data.frame"), prototype = list(newdata = NULL, rescaling = "none", level = 0.95), contains = "textmodel_wordscores_fitted")
...
#' @export
textmodel_wordscores <- function(data, scores, scale = c("linear", "logit"), smooth = 0) {
    ...
    new("textmodel_wordscores_fitted", Sw = Sw, x = data, y = scores, method = "wordscores", scale = scale, call = match.call())
}
...
predict.textmodel_wordscores_fitted <- function(object, newdata = NULL, rescaling = "none", level = 0.95, verbose = TRUE, ...) {
    if (length(list(...)) > 0) 
    ...
    if (verbose) 
        catm(paste(length(scorable), " of ", nfeature(data), " features (", round(100 * length(scorable)/nfeature(data), 2), "%) can be scored\n\n", sep = ""))
    
    # compute text scores as weighted mean of word scores in 'virgin' document Fw <- tf(data) # first compute relative term weights scorable.newd <- Fw[, featureIndex] # then exclude any features not found/scored
    ...
    for (i in 1:length(textscore_raw_se)) textscore_raw_se[i] <- sqrt(sum(Fwv[i, , drop = FALSE] * (textscore_raw[i] - Sw)^2))/sqrt(rowSums(scorable.newd)[i])
    
    z <- stats::qnorm(1 - (1 - level)/2)
    
    result <- data.frame(textscore_raw, textscore_raw_se, textscore_raw_lo = textscore_raw - z * textscore_raw_se, textscore_raw_hi = textscore_raw + z * textscore_raw_se)
    ...
    if ("mv" %in% rescaling) {
        ...
        textscore_mv <- (textscore_raw - textscore_raw[lowerIndex]) * (max(object@y, na.rm = TRUE) - min(object@y, na.rm = TRUE))/(textscore_raw[upperIndex] - textscore_raw[lowerIndex]) + min(object@y, na.rm = TRUE)
    ...
    if ("lbg" %in% rescaling) {
        ...
        result <- cbind(result, data.frame(textscore_lbg, textscore_lbg_lo, textscore_lbg_hi))
    }
    
    new("textmodel_wordscores_predicted", rescaling = rescaling, newdata = newdata, textscores = result)
}
...
print.textmodel_wordscores_fitted <- function(x, n = 30L, digits = 2, ...) {
    ...
    refscores <- data.frame(Documents = docnames(x@x), `Ref scores` = x@y)
...
summary.textmodel_wordscores_fitted <- function(object, ...) {
    ...
    dd <- data.frame(Score = object@y, Total = apply(object@x, 1, sum), Min = apply(object@x, 1, min), Max = apply(object@x, 1, max), Mean = apply(object@x, 1, mean), Median = apply(object@x, 1, stats::median))
...
print.textmodel_wordscores_predicted <- function(x, ...) {
    ...
    ## options if a wordscores object (and no, it's not very object-oriented!)
...
# #wordscores require(quantedaData) data(ie2010Corpus) ieDfm <- dfm(ie2010Corpus) refData <- ieDfm[5:8,] # Cowen and Kenny refScores <- c(-1,1,NA,NA) fitModel(ieDfm, refScores=c(-1,1), scale='logit')

# mod <- fitModel(refScores ~ refData, method='wordscores', scale='logit') # naive bayes require(quantedaData) data(ie2010Corpus) ieDfm <- dfm(ie2010Corpus) trainData <- ieDfm[5:6,] # Cowen and Kenny trainLabels <- c(-1,1) smooth<-1 labels <- levels(as.factor(trainLabels)) trainData <-
# trainData + smooth PwGc <- rowNorm(trainData) PcGw <- colNorm(PwGc * outer(1/length(trainLabels), rep(1, ncol(PwGc))))
