...
#'     head(testw)
#' }}
setMethod("weight", signature = c("dfm", "character"), definition = function(x, type = c("frequency", "relFreq", "relMaxFreq", "logFreq", "tfidf"), ...) {
...
setMethod("weight", signature = c("dfm", "numeric"), definition = function(x, type, ...) {
    weights <- type
...
# #' @rdname weight #' @export setGeneric('weighting', function(object) standardGeneric('weighting'))
...
setGeneric("docfreq", function(x, scheme = c("count", "inverse", "inversemax", "inverseprob", "unary"), smoothing = 0, k = 0, base = 10, threshold = 0, USE.NAMES = TRUE) standardGeneric("docfreq"))

#' @rdname docfreq
setMethod("docfreq", signature(x = "dfm"), function(x, scheme = c("count", "inverse", "inversemax", "inverseprob", "unary"), smoothing = 0, k = 0, base = 10, threshold = 0, USE.NAMES = TRUE) {
    ...
    if (scheme == "unary") {
        ...
        if (is(x, "dfmSparse")) {
            ...
            if (!any(x@x <= threshold)) 
                result <- rep(ndoc(x), nfeature(x)) else result <- colSums(as.matrix(x) > threshold)
        }
...
# #' @rdname docfreq #' @export setGeneric('df', function(x, ...) standardGeneric('df')) #' @rdname docfreq #' @export setMethod('df', signature(x = 'dfm'), function(x, ...) docfreq(x, ...))
...
#' @export
setGeneric("tf", function(x, scheme = c("count", "prop", "propmax", "boolean", "log", "augmented", "logave"), base = 10, K = 0.5) standardGeneric("tf"))

#' @rdname tf
setMethod("tf", signature(x = "dfm"), definition = function(x, scheme = c("count", "prop", "propmax", "boolean", "log", "augmented", "logave"), base = 10, K = 0.5) {
    ...
    if (scheme == "count") {
        ...
        div <- rowSums(x)
        if (is(x, "dfmSparse")) 
            x@x <- x@x/div[x@i + 1] else x <- x/div
        ...
        maxtf <- maxtf(x)
        if (is(x, "dfmSparse")) 
            x@x <- K + (1 - K) * x@x/maxtf[x@i + 1] else x <- K + (1 - K) * x/maxtf
        x@weightTf[["K"]] <- K
        
    } else if (scheme == "logave") {
        meantf <- Matrix::rowMeans(x)
        if (is(x, "dfmSparse")) 
            x@x <- (1 + log(x@x, base))/(1 + log(meantf[x@i + 1], base)) else x <- (1 + log(x, base))/(1 + log(meantf, base))
        x@weightTf[["base"]] <- base
...
## internal function to get maximum term frequency by document only applies to CsparseMatrix formats (dfmSparse)
...
setMethod("maxtf", signature(x = "dfmSparse"), definition = function(x) {
    ...
    ## note: this is faster for small dfms: sapply(split(x@x, x@i), max)
})
...
# require(data.table) f1 <- function(x) { dt <- data.table(doc = t(x)@i, freq = x@x) dt[, max(freq), by = doc][, V1] } f2 <- function(x) { sapply(split(x@x, x@i), max) } myDfm <- dfm(inaugCorpus, verbose = FALSE) microbenchmark(f1(myDfm), f2(myDfm)) microbenchmark(f1(NSFdfm), f2(NSFdfm),
# times = 5)


# #' @rdname tfidf #' @param x object for which idf or tf-idf will be computed (a document-feature #' matrix) #' @param k additional constant to add to the denominator in \emph{docfreq} #' computation, default is \code{k = 1} #' @param base the base with respect to which logarithms in
# \emph{idf} #' and \emph{idf} computation are computed, default is 10 (see Manning, #' Raghavan, and Schutze 2008, p123).  #' @param USE.NAMES logical; if \code{TRUE} attach feature labels as names of #' the resulting numeric vector #' @export #' @details \code{idf} computes inverse
# document frequency with a constant #' \code{k} added to the denominator of log document frequency.  #' @seealso \code{\link{docfreq}}, \code{\link{weight}} idf <- function(x, k = 1, USE.NAMES = TRUE, base = 10) UseMethod('idf') #' @rdname tfidf #' @export idf.dfm <- function(x, k = 1,
# USE.NAMES = TRUE, base = 10) { x <- log(ndoc(x), base = base) - log(docfreq(x, USE.NAMES = FALSE) + k, base = base) if (!USE.NAMES) names(x) <- NULL x }
