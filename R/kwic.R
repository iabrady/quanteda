...
kwic.tokenizedTexts <- function(x, keywords, window = 5, valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, ...) {
    ...
    if (sum(is.na(contexts)) == length(contexts)) 
        return(NA)  # means no search term found
...
kwic.tokenizedText <- function(x, word, window = 5, valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE) {
    ...
    if (valuetype == "fixed") {
        if (case_insensitive) 
            startMatchIndex <- matchFixed(x, word, case_insensitive) else startMatchIndex <- match(x, word)
    ...
    indexPost <- cbind(ifelse(endMatchIndex + wordLength < length(x), endMatchIndex + wordLength, NA), pmin(endMatchIndex + wordLength + window - 1, length(x)))
    concatIndexes <- function(indexPair, textVector) {
        if (any(is.na(indexPair))) 
            return("")
        if (identical(indexPair, c(0, 0))) 
            return("")
        if (length(indexPair) == 1) 
            textVector[indexPair] else paste(textVector[indexPair[1]:indexPair[2]], collapse = " ")
    }
    
    result <- data.frame(position, contextPre = apply(indexPre, 1, concatIndexes, x), keyword = apply(indexKeyword, 1, concatIndexes, x), contextPost = apply(indexPost, 1, concatIndexes, x), stringsAsFactors = FALSE)
...
## solution from alexis_laz http://stackoverflow.com/questions/33027611/how-to-index-a-vector-sequence-within-a-vector-sequence
matchSequence = function(pat, x) {
    ff = function(.pat, .x, acc = if (length(.pat)) 
        seq_along(.x) else integer(0)) {
        if (!length(.pat)) 
            return(acc)
...
# helper function for matchSequence wraps a vector by removing the first n elements and padding end with NAs
wrapVector <- function(x, n) {
    stopifnot(n <= length(x))
    if (n == length(x)) 
        return(rep(NA, n)) else return(c(x[(n + 1):length(x)], rep(NA, n)))
}
...
matchFixed <- function(x, table, case_insensitive) {
    ...
    # result <- rep(NA, length(x)) for (i in 1:length(table)) { # logMatch <- grepl(table[i], x) logMatch <- !is.na(stringi::stri_locate_first_fixed(x, table[i], opts_fixed = list(case_insensitive = case_insensitive))[, 1]) result[which(logMatch)] <- i } result
}
