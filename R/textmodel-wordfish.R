...
#' @export
setClass("textmodel_wordfish_fitted", slots = c(priors = "numeric", tol = "numeric", dir = "numeric", theta = "numeric", beta = "numeric", psi = "numeric", alpha = "numeric", phi = "numeric", docs = "character", features = "character", sigma = "numeric", ll = "numeric", dispersion = "character", 
    se.theta = "numeric"), contains = "textmodel_fitted")

#' @rdname textmodel_fitted-class
#' @export
setClass("textmodel_wordfish_predicted", slots = c(newdata = "dfm", level = "numeric", predvals = "data.frame"), prototype = list(level = 0.95), contains = "textmodel_wordfish_fitted")
...
#' }}
#' @export
textmodel_wordfish <- function(data, dir = c(1, 2), priors = c(Inf, Inf, 3, 1), tol = c(1e-06, 1e-08), dispersion = c("poisson", "quasipoisson"), dispersionLevel = c("feature", "overall"), dispersionFloor = 0) {
    ...
    if (length(zeroLengthDocs) | length(zeroLengthFeatures)) 
        catm("\n")
    ...
    if (dispersionFloor < 0 | dispersionFloor > 1) 
    ...
    # if (length(addedArgs <- list(...))) warning('Argument', ifelse(length(addedArgs)>1, 's ', ' '), names(addedArgs), ' not used.', sep = '')
    
    # check quasi-poisson settings and translate into numerical values 1 = Poisson, 2 = quasi-Poisson, overall dispersion, 3 = quasi-Poisson, term dispersion, 4 = quasi-Poisson, term dispersion w/floor
    if (dispersion == "poisson") 
        disp <- 1L else if (dispersion == "quasipoisson" & dispersionLevel == "overall") 
        disp <- 2L else if (dispersion == "quasipoisson" & dispersionLevel == "feature") {
        if (dispersionFloor) 
            disp <- 4L else disp <- 3L
    } else stop("Illegal option combination.")
    
    # catm('disp = ', disp, '\n')
    
    wfresult <- wordfishcpp(as.matrix(data), as.integer(dir), 1/(priors^2), tol, disp, dispersionFloor)
    # NOTE: psi is a 1 x nfeature matrix, not a numeric vector alpha is a ndoc x 1 matrix, not a numeric vector
    new("textmodel_wordfish_fitted", x = data, docs = docnames(data), features = features(data), dir = dir, dispersion = dispersion, priors = priors, theta = wfresult$theta, beta = wfresult$beta, psi = as.numeric(wfresult$psi), alpha = as.numeric(wfresult$alpha), phi = as.numeric(wfresult$phi), 
        se.theta = wfresult$thetaSE, call = match.call())
}
...
print.textmodel_wordfish_fitted <- function(x, n = 30L, ...) {
    ...
    results <- data.frame(Documents = docnames(x@x), theta = x@theta, SE = x@se.theta, lower = x@theta - 1.96 * x@se.theta, upper = x@theta + 1.96 * x@se.theta)
    print(results, ...)
    if (n > 0) {
...
summary.textmodel_wordfish_fitted <- function(object, ...) {
    ...
    cat("\nEstimated document positions:\n")
    results <- data.frame(theta = object@theta, SE = object@se.theta, lower = object@theta - 1.96 * object@se.theta, upper = object@theta + 1.96 * object@se.theta)
...
# @rdname textmodel_wordfish @export @method print textmodel_wordfish_predicted print.textmodel_wordfish_predicted <- function(x, ...) { cat('Predicted textmodel of type: wordfish\n\n') #cat('Call:\n\t') #print(object$call) x <- x@textfish ## options if a wordscores object ## (and no,
# it's not very object-oriented!)  names(x)[which(names(x)=='textscore_raw')] <- 'textscore' names(x)[which(names(x)=='textscore_raw_se')] <- 'LBG se' names(x)[which(names(x)=='textscore_raw_lo')] <- 'ci lo' names(x)[which(names(x)=='textscore_raw_hi')] <- 'ci hi'
# names(x)[which(names(x)=='textscore_mv')] <- 'MV rescaled' names(x)[which(names(x)=='textscore_lbg')] <- 'LBG rescaled' names(x)[which(names(x)=='textscore_lbg_lo')] <- 'LBG lo' names(x)[which(names(x)=='textscore_lbg_hi')] <- 'LBG hi' # pare down the output if rescaling has been specified
# if (any(c('LBG rescaled', 'MV rescaled') %in% names(x))) x$ci.lo <- x$ci.hi <- NULL print(round(as.data.frame(x), 4)) cat('\n') }
