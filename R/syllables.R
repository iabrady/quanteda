...
syllables.tokenizedTexts <- function(x, syllableDict = quanteda::englishSyllables, ...) {
    ...
    syllablesDT <- data.table(docIndex = rep(1:length(x), lengths(x)), word = unlist(x), serial = 1:length(unlist(x)))
...
syllables.data.table <- function(x, syllableDict = quanteda::englishSyllables, ...) {
    ...
    if (is.null(syllableDict)) {
        # data(englishSyllables, envir = environment()) syllableDict <- englishSyllables
    } else {
    ...
    # merge words to get syllables suppressWarnings so it won't complain about mixed encodings
