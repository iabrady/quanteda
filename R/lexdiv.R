...
lexdiv.dfm <- function(x, measure = c("all", "TTR", "C", "R", "CTTR", "U", "S", "Maas"), log.base = 10, drop = TRUE, ...) {
    ...
    # nTokens <- ntoken(x) nTypes <- ntype(x)
    ...
    if ("S" %in% measure) 
        results[, `:=`(S, log(log(nTypes, base = log.base), base = log.base)/log(log(nTokens, base = log.base), base = log.base))]
    
    if ("Maas" %in% measure) {
        measure <- c(measure, "lgV0", "lgeV0")
        results[, `:=`(Maas, sqrt((log(nTokens, base = log.base) - log(nTypes, base = log.base))/log(nTokens, base = log.base)^2))]
