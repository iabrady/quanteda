...
#' @export
selectFeatures.dfm <- function(x, features, selection = c("keep", "remove"), valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, verbose = TRUE, ...) {
    ...
    if (valuetype == "regex") {
        if (all.equal(x@ngrams, 1L) == TRUE) {
            featIndex <- which(stringi::stri_detect_regex(features_x, paste0(features, collapse = "|"), case_insensitive = case_insensitive, ...))
        } else {
            #### 
            matchPattern <- paste0(features, collapse = "|")
            featIndex <- which(sapply(features_x, function(x) any(stringi::stri_detect_regex(x, matchPattern, case_insensitive = case_insensitive, ...))))
        }
    } else {
        if (all.equal(x@ngrams, 1L) == TRUE) 
            featIndex <- which(features(x) %in% features)  # unigrams
 else featIndex <- which(sapply(features_x, function(f) any(f %in% features), USE.NAMES = FALSE))  # ngrams
    }
    
    if (verbose & !features_from_dfm) 
        catm(ifelse(selection == "keep", "kept", "removed"), " ", format(length(featIndex), big.mark = ","), " feature", ifelse(length(featIndex) > 1 | length(featIndex) == 0, "s", ""), ", from ", length(features), " supplied (", originalvaluetype, ") feature type", ifelse(length(features) > 
            0 | length(featIndex) == 0, "s", ""), "\n", sep = "")
    
    # pad the zeros if features was a dfm, return in same feature order as original dfm for selection = 'keep' only
    if (features_from_dfm) {
        
        if (verbose) 
            catm(ifelse(selection == "keep", "found", "zeroed"), " ", format(length(featIndex), big.mark = ","), " feature", ifelse(length(featIndex) > 1 | length(featIndex) == 0, "s", ""), " from ", length(features), " supplied type", ifelse(length(features) > 0 | length(featIndex) == 0, "s", 
                ""), " in a dfm,", sep = "")
        ...
        if (verbose) 
            catm(" padding 0s for another", length(origDfmFeatureIndex), "\n")
        x <- new("dfmSparse", Matrix::cbind2(x2, sparseMatrix(i = NULL, j = NULL, dims = c(ndoc(x2), length(origDfmFeatureIndex)), dimnames = list(docnames(x2), features[origDfmFeatureIndex]))))
        # matrix(0, nrow = ndoc(x2), ncol = length(origDfmFeatureIndex)))) x <- cbind(x2, ) colnames(x2)[(xOriginalFeatureLength + 1) : nfeature(x2)] <- features[origDfmFeatureIndex]
    ...
    if (selection == "keep") 
        return(x[, featIndex]) else return(x[, -featIndex])
}
...
selectFeatures.tokenizedTexts <- function(x, features, selection = c("keep", "remove"), valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, padding = FALSE, indexing = FALSE, verbose = FALSE, ...) {
    selection <- match.arg(selection)
    valuetype <- match.arg(valuetype)
    originalvaluetype <- valuetype
    ...
    if (valuetype == "fixed") {
        ...
        if (verbose) 
            catm(sprintf("Scanning %.2f%% of texts...\n", 100 * sum(flag)/n))
        if (selection == "remove") {
        ...
        if (verbose) 
            catm("Converting regex to fixed...\n")
...
#' @export
selectFeatures.collocations <- function(x, features, selection = c("keep", "remove"), valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, verbose = TRUE, pos = 1:3, ...) {
    ...
    nend <- nrow(x)
    if (verbose) 
        catm("Removed ", format(nstart - nend, big.mark = ","), " (", format((nstart - nend)/nstart * 100, digits = 3), "%) of ", format(nstart, big.mark = ","), " collocations containing one of ", length(features), " stopwords.\n", sep = "")
    class(x) <- origclass
    x
}
...
# @rdname selectFeatures @examples ## example for collocations (myCollocs <- collocations(inaugTexts[1:3], n=20)) selectFeatures(myCollocs, stopwords('english'), 'remove') @export
selectFeatures_collocations <- function(x, features, selection = c("keep", "remove"), valuetype = c("glob", "regex", "fixed"), case_insensitive = TRUE, verbose = TRUE, pos = 1:3, ...) {
    
    # necessary to pass by value - inefficient but 'R-like' xloc <- data.table::copy(x)
    ...
    if (valuetype == "fixed") {
        ...
        x[, `:=`(paste0("i", pos), lapply(x[, pos, with = FALSE], function(y) {
            if (selection == "remove") 
                !(y %in% features) else (y %in% features)
        }))]
        ...
        x[, `:=`(paste0("i", pos), lapply(x[, pos, with = FALSE], function(y) {
            if (selection == "remove") 
                !stringi::stri_detect_regex(y, paste0(features, collapse = "|"), case_insensitive = case_insensitive, ...) else stringi::stri_detect_regex(y, paste0(features, collapse = "|"), case_insensitive = case_insensitive, ...)
        }))]
...
# x <- data.table::copy(myCollocs) features <- stopwords('english') selection = c('remove') valuetype = c('glob') case_insensitive = TRUE verbose = TRUE pos = 1:3

# require(microbenchmark) myCollocs <- collocations(inaugCorpus, size=2:3) microbenchmark( old = removeFeatures(myCollocs, stopwords('english'), verbose = FALSE), new = selectFeatures(myCollocs, stopwords('english'), 'remove'), unit = 'relative', times = 30 )

# Convert regex to fixed @param regex regular expression @param types unique types of tokens @param case_insensitive case sensitivity @param ... additional arguments passed to \code{\link[stringi]{stri_detect_regex}} @export
