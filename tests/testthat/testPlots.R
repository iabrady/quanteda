...
test_that("test plot.kwic facet order parameter", {
    ...
    expect_true(all(plot_docnames[order(plot_docnames)] == plot_docnames))
    ...
    plot_docnames <- as.character(ggplot2::ggplot_build(p)$panel$layout$docname)
    expect_false(all(plot_docnames[order(plot_docnames)] == plot_docnames))
...
test_that("test plot.kwic keeps order of keywords passed", {
    ...
    expect_equal(as.character(unique(ggplot2::ggplot_build(p)$panel$layout$keyword)), c("people", "american"))
})
