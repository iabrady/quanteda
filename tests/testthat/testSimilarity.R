...
test_that("test similarity method = \"correlation\" against base cor()", {
    presDfm <- dfm(subset(inaugCorpus, Year > 1980), ignoredFeatures = stopwords("english"), stem = TRUE, verbose = FALSE)
...
test_that("test similarity method = \"cosine\" against proxy simil()", {
    ...
    ## NOT EQUAL expect_equal(cosQuanteda[1:10], cosProxy[2:11]) #, cosQlcMatrix[2:11])
})
...
test_that("test similarity method = \"cosine\" against proxy simil(): documents", {
    ...
    cosProxy <- round(proxy::as.matrix(proxy::simil(as.matrix(presDfm), "cosine", by_rows = TRUE), diag = 1), 6)
...
test_that("simple similarity comparisons method = \"cosine\" against proxy simil()", {
    testText <- c("The quick brown fox named Seamus jumps over the lazy dog also named Seamus, with the newspaper from a boy named quick Seamus, in his mouth.", "the quick brown Seamus named dog jumped.", "My lazy dog who looks like a quick fox was in the newspaper.")
...
# sort(as.matrix(proxy::simil(as.matrix(d), as.matrix(d[, 'seamus']), 'cosine', by_rows = FALSE))[, 1], decreasing = TRUE)[-2] similarity(d, 'seamus', method = 'cosine')[['seamus']]
